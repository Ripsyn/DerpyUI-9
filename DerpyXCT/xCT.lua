...
if(ct.damagestyle)then
	DAMAGE_TEXT_FONT  = M.media.fontn_lang
...
local on_push = function(self,button)
	...
	if button == "RightButton" then
		DerpyXCT[a] = help_sw_r[DerpyXCT[a]]
	else
		DerpyXCT[a] = help_sw_l[DerpyXCT[a]]
	end
	self.m:SetText(DerpyXCT[a])
end
...
local StartConfigmode=function()
	if not InCombatLockdown()then
		for i=1,#ct.frames do
			...
			f.fs = M.setfont_lang(f,15,nil,nil,"CENTER","BOTTOM")
			f.fs:SetHeight(50)
...
SlashCmdList["XCT"]=function(input)
	...
	elseif(input=="reset")then
		...
		xCT3:SetWidth(216)
		xCT4:ClearAllPoints()
		xCT4:SetPoint("CENTER",UIParent, "CENTER", 244, 152)
		xCT4:SetHeight(172)
		xCT4:SetWidth(136)
		for i=1,4 do DerpyPosVars["xCT"..i] = nil end
	else

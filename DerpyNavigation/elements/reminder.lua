...
local function OnAuraChange(self, event, arg1, unit)
	...
	for i, Spell6Buff in pairs(Spell6Buff) do
		...
		if spellname and UnitAura("player", spellname) then
			Spell6Frame:Disable()
...
local Enable = function(self)
	...
	self.enabled = true
	self.t:SetGradient("VERTICAL",.345,.345,.345,1,1,1)
end

local Disable = function(self)
	if self.enabled ~= true then return end
	self.enabled = false
	self.t:SetGradient("VERTICAL",.1,.1,.1,.345,.345,.345)
end
...
FoodFrame.t:SetGradient("VERTICAL",.345,.345,.345,1,1,1)
...
M.checkvarleft = function()
	Right_Nav:check_var()
	Right_Nav:EnableMouse(true)
end

M.addenter(function()
	Right_Nav:EnableMouse(false)
end)

...
local ft = M["media"].font_lang -- Map font
local fontsize = 22 -- Map Font Size
local _update_coord_savedvars = M.null
local update_coord_show = update_coord_show
...
local current_local_name = M.setfont_lang(movebutton,13,nil,nil,"CENTER")
...
coord_frame:SetPoint("TOPLEFT",mapbg,4,-4)
coord_frame:SetPoint("BOTTOMRIGHT",mapbg,-4,4)
...
local t_1,t_2,t_3 = 0,1,0
local y_pl,y_pl_t1,y_pl_t2 = mk_str(t_1,t_2,t_3,"ARTWORK","BOTTOMLEFT","TOPRIGHT",2,3,-1,0)
local x_pl,x_pl_t1,x_pl_t2 = mk_str(t_1,t_2,t_3,"ARTWORK","TOPLEFT","BOTTOMRIGHT",2,1,-2,3)
...
do
	...
	for i=1,4 do
		...
		local text = M.setfont_lang(f,12)
...
movebutton:SetScript("OnMouseDown", function(self,bt)
...
movebutton:SetScript("OnMouseUp", function(self,bt)
	if bt ~= "RightButton" then
...
M.addlast(function()
	...
	WorldMapFrameSizeUpButton:SetNormalTexture(nil)
	WorldMapFrameSizeUpButton:SetHighlightTexture(nil)
	WorldMapFrameSizeUpButton:SetPushedTexture(nil)
...
local _tex1,_tex2,_tex3
do
	_tex1 = WorldMapFrameCloseButton:GetNormalTexture():GetTexture()
	_tex2 = WorldMapFrameCloseButton:GetHighlightTexture():GetTexture()
	_tex3 = WorldMapFrameCloseButton:GetPushedTexture():GetTexture()
end

local crssss = function(mode)
	if mode then
		WorldMapFrameCloseButton:SetNormalTexture(nil)
		WorldMapFrameCloseButton:SetHighlightTexture(nil)
		WorldMapFrameCloseButton:SetPushedTexture(nil)
	else
		WorldMapFrameCloseButton:SetNormalTexture(_tex1)
		WorldMapFrameCloseButton:SetHighlightTexture(_tex2)
		WorldMapFrameCloseButton:SetPushedTexture(_tex3)
	end
end
...
local SmallerMapSkin = function()
	...
	WorldMapQuestShowObjectives:SetAlpha(0)
	style_AreaFrame(4,-4)
	crssss(true)
	WorldMapQuestShowObjectives:EnableMouse(false)
...
local BiggerMapSkin = function()
	...
	WorldMapQuestShowObjectives:SetAlpha(1)
	WorldMapQuestShowObjectives:EnableMouse(true)
	WorldMapShowDigSites:SetAlpha(1)
...
do
	local temp_obj
	local WorldMapFrameSizeDownButton = WorldMapFrameSizeDownButton
...
do
	...
	local flo = function(x) return floor(x+.5) end
	local show_player_coord,show_cursor_coord
	local GetPlayerMapPosition = GetPlayerMapPosition;
	local the_n = 0
	local coord_frame = coord_frame
	...
	local _update_cursor = function()
		...
		y_cur:SetPoint("TOPLEFT",coord_frame,"TOPLEFT",0,-y)
		y_cur:SetPoint("TOPRIGHT",coord_frame,"TOPRIGHT",0,-y)
		x_cur:SetPoint("TOPLEFT",coord_frame,"TOPLEFT",x,0)
		x_cur:SetPoint("BOTTOMLEFT",coord_frame,"BOTTOMLEFT",x,0)
	...
	local _update_player = function()
		...
		--x = flo(x); y = flo(y)
		y_pl:SetPoint("TOPLEFT",coord_frame,"TOPLEFT",0,-y)
		y_pl:SetPoint("TOPRIGHT",coord_frame,"TOPRIGHT",0,-y)
		x_pl:SetPoint("TOPLEFT",coord_frame,"TOPLEFT",x,0)
		x_pl:SetPoint("BOTTOMLEFT",coord_frame,"BOTTOMLEFT",x,0)
		local tx,ty = flo(x/width*100),flo(y/height*100)
		y_pl_t1:SetText(ty)
		y_pl_t2:SetText(ty)
		x_pl_t1:SetText(tx)
	...
	update_coord_show = function()
		if not(show_cursor_coord) and not(show_player_coord) then
			coord_frame:Hide()
		else	
			coord_frame:Show()
		end		
...
M.addafter(function()
	local x1,_,x3,x4,x5 = WorldMapScreenAnchor:GetPoint()
	WorldMapScreenAnchor:SetPoint(x1,UIParent,x3,x4/M.scale,x5/M.scale)
end)
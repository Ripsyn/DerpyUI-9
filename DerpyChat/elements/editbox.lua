...
edit.parent.__SetPoint = edit.parent.SetPoint
edit.parent.SetPoint = function(self,...)
	self:__SetPoint(...)
	self.edit:SetWidth(floor(self:GetWidth()+.5))
end
...
function edit.colorize(edit,r,g,b)
...
hooksecurefunc("ChatEdit_UpdateHeader", function()
	local type = ChatFrame1EditBox:GetAttribute("chatType")
		...
		local id = GetChannelName(ChatFrame1EditBox:GetAttribute("channelTarget"))
			if id == 0 then
				edit:backcolor(0,0,0)
			else
				edit:colorize(ChatTypeInfo[type..id].r,ChatTypeInfo[type..id].g,ChatTypeInfo[type..id].b)
			end
		else
			edit:colorize(ChatTypeInfo[type].r,ChatTypeInfo[type].g,ChatTypeInfo[type].b)
		end
end)

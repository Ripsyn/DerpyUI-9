...
local function CreatCopyFrame()
	...
	local title = M.setfont_lang(frame,16)
...
local function GetLines(...)
	...
	for i = select("#", ...), 1, -1 do
		...
		if region:GetObjectType() == "FontString" then
			lines[ct] = tostring(region:GetText()):gsub("|T.-|t","")
			ct = ct + 1
		end
	end
	return ct - 1
...
local function Copy(cf,name)
	...
	FCF_SetChatWindowFontSize(cf, cf, 0.001)
...
local MakeCopyButton = function(i,cf)
	if not cf then cf = _G["ChatFrame"..i] end

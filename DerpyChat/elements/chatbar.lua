...
local _t = M.setfont_lang(fade_chan_name,18,nil,nil,"CENTER")
...
local setbutton = function(lang_st,event,chan,tosay,enable,num)
...
--load and check
local load_all_stuff = function(lang_st)
	get_chan = nil
	...
	for i=1,#chatbar_table do
		if chatbar_table[i] then
			local y = setbutton(lang_st,unpack(chatbar_table[i]))
...
chatbar.n = 2
M.addlast(function()
	chatbar:SetScript("OnUpdate",function(self,t)
	self.n = self.n - 1

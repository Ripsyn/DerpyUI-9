...
local force_size_ = V["force_font_size"]
local font_size_ = V["font_size"]
...
M.addenter(function()
	...
	_G.ChatMenu.sbutn = sbutn
	_G.ChatMenu.sbut = sbut
	M.ChangeTemplate(_G.ChatMenu) 
	_G.ChatMenu:ClearAllPoints()
	_G.ChatMenu:SetPoint("BOTTOMLEFT",sbut,"TOPLEFT",0,-2) 
	
	_G.ChatMenu:SetScript("OnShow",function(self)  
		self.sbut:backcolor(1, 0.8, 0.15, .5)
		self.sbutn:SetTextColor(1, 0.8, 0.15)
	end)
	
	_G.ChatMenu:SetScript("OnHide",function(self) 
		self.sbut:backcolor(0,0,0)
		self.sbutn:SetTextColor(1, 1, 1)
	end)
	
	ChatFrameMenuButton:SetAllPoints(sbut)
	ChatFrameMenuButton:SetAlpha(0)
	ChatFrameMenuButton.sbutn = sbutn
	ChatFrameMenuButton:SetScript("OnEnter", function(self) 
		self.sbutn:SetTextColor(1, 0.8, 0.15)
	end)
	ChatFrameMenuButton:SetScript("OnLeave", function(self) 
	...
	for i = 1, NUM_CHAT_WINDOWS do
		...
		M.addlast(function()
			...
			frame:SetFont(M["media"].fontn_lang, x)
		end)
		...
		frame:SetScale(1)
		frame:SetJustifyH(left_j)
	end
	
	GeneralDockManager:ClearAllPoints()
	GeneralDockManager:SetPoint("BOTTOMLEFT",FirstFrameChat,"TOPLEFT",0,3.2)
	GeneralDockManager:SetPoint("BOTTOMRIGHT",FirstFrameChat,"TOPRIGHT",-36,3.2)
...
M.addlast(function()
	...
	if ch ~= 0 and ch ~= 1 and ch ~= -1 and ch then
		a = ch
...
-- Sound
local Sound = CreateFrame("Frame")
local PlaySoundFile = PlaySoundFile
local file = M["media"].path.."cathedral.mp3"
Sound:RegisterEvent("CHAT_MSG_WHISPER")
Sound:RegisterEvent("CHAT_MSG_BN_WHISPER")
Sound:SetScript("OnEvent", function()

...
local stop = function(self) 
	if self.combat and InCombatLockdown() then return end
...
M.make_movable = function(self,t)
	...
	self:SetScript("OnMouseDown",start)
	self._stooooop = stop
	self:SetScript("OnMouseUp",stop)
	if t then
		self:EnableMouse(true)
	end
end
...
local unlock_pos_now = function()
	for i=1, #movers_table do
		...
		if a.combat == true and InCombatLockdown() then
			print(a.realname.." blocked -- combat...")
		else
			a:EnableMouse(true);
			a:Show();
			a.t1:Show();
...
local lock_pos_now = function(as)
	for i=1, #movers_table do
		local a = movers_table[i]
		if not(a.combat and InCombatLockdown()) then
			a:EnableMouse(false);
			a:t3()
			a.t1:Hide();
			a.t2:Hide();

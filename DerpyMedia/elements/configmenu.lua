...
title:SetPoint("TOP",top_cir,.3,-4)
title:SetText("DERPY")

local bottitle = M.setfont(bg,20,nil,"OVERLAY")
bottitle:SetPoint("BOTTOM",top_cir,.3,5)
...
local alphaisalpha = M.setfont(bg,25,nil,"OVERLAY","CENTER")
alphaisalpha:SetHeight(26*2)
alphaisalpha:SetPoint("CENTER",bot_cir,-.3,1)
alphaisalpha:SetText("|cffffffffSTEP.8\n16.02.2012|r")
...
M.addenter(function()
	--mainframe:gen_button(nil,"MOVE SOMETHING",M.switch_lock)
	M.pos_config_do()
	if M.hover_bind_call then
		mainframe:gen_button(nil,"HOVERBIND",M.hover_bind_call)
	end
...
right_bg.run_height = function(self,x)
	...
	if a <= x then
		self.hspeed = a==0 and 2000 or 400
		self.hmod = 1
	else
		self.hspeed = -400
		self.hmod = -1
